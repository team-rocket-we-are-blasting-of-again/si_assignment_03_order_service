...
use crate::students::{
    book_purchase_service_client::BookPurchaseServiceClient, 
    StudentInfo, 
    BookToBuy,
    BoughtBookReply,
};
...
pub async fn order(book_id: i32, student_id: i32, price: f64) -> Result<BoughtBookReply, Box<dyn std::error::Error>>{
    let url = env::var("STUDENT_SERVICE_URL").unwrap();
    let mut client = BookPurchaseServiceClient::connect(url).await?;
    let request = tonic::Request::new(
        BookToBuy {
